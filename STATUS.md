...
- **`multer` with `memoryStorage`**, capped at 4 MB / 1 file. No temp files to clean up on any failure path. **`express.json({ limit: "1mb" })` does NOT apply to uploads** — body parsers are content-type-gated, so `multer`'s `limits.fileSize` is the only bound that exists. Getting this wrong means an unbounded upload.
